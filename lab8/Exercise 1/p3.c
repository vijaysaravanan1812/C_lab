...
void enqueue(struct queue *q , int x)
{
    ...
    else
    {
        if (q ->rear == -1 && q ->front == -1)
        {
            ...
            q -> front = 0;//at first front and rear point to same location 
            printf("%d\n",q->rear);
        }
        else
        {
            ...
            printf("%d\n",q->rear);
            //Do not distrub the front and 
            //increase the rear
        }
        q ->arr[q ->rear] = x; 
        // to display queue
        for (int i =  q ->rear; i <= q->front; i = (i+1) % q->capacity)
        {
            printf("%d\t",q->arr[i]);
        }printf("\n");
    }
...
int dequeue(struct queue *q)
{
    ...
    else
    {
        ...
        if (q ->front != -1)
        {
            for (int i =  q ->rear; i <= q->front; i = i % q->capacity)
            {
                printf("%d\t",q->arr[i]);
                i = (i + 1) % q ->capacity;
            }printf("\n");
